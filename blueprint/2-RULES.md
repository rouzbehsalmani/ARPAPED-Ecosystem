...
| **Registry** | The canonical discovery/index service: capability+operation → bounded implementation candidates. There is exactly one canonical Registry per runtime (same scoping as Bridge, above) — never one shared across independent applications, and never a second one competing within a runtime that already has its resolved canonical one. |
| **Policy** | The canonical authorization stage that evaluates each candidate against the request's policy context, per runtime (same scoping as Bridge). |
...
| **runtime event** | One real Bridge call, success or failure, recorded by `blueprint.dep.runtime_log` (`runtime-event.schema.json`) — distinct from **verification record**: that record is written once per verified CYCLE, by a harness; this event is written once per CALL, by the Bridge itself (an opt-in `event_sink` on `Bridge.__init__`), for every real call it ever handles — harness-scripted or genuine production traffic alike, including a nested call a factory/process executor makes through the same Bridge (R4), each its own independent event. Carries the same `trace`/`selection`/`evidence`/`input` shapes a verification check already does — an empty `trace` is honest (zero stages reached before `BridgeRequest.validate()` itself failed), never padded to look like more happened. |
| **resolution (runtime event)** | An EXPLICIT record of how a prior **runtime event** failure was actually fixed (`RuntimeEventLog.record_resolution`, same `runtime-event.schema.json`) — linked to that failure by its own `event_id`, never guessed by matching capability_id/operation or timestamp proximity. Never auto-inferred from a later success (a later success could be coincidence — different input, a transient dependency recovering — so only a deliberate act by whoever diagnosed and fixed it counts), the same discipline agent-cycle-report.schema.json's decisions[].`failure`/`correction` already holds for cycle-level failures, applied here at call granularity. Optionally names `verified_by_event_id` (a later success event proving the fix worked) and `commit` (the git commit that made it, when there was one — optional, same posture as **resulting state ref**'s `vcs`). |
| **git-commit event** | The fact that a commit was made, recorded by `blueprint.dep.git_commit_event` (`git-commit-event.schema.json`) — independent of **runtime event** (own module, own shape, no shared code): a commit is an agent/operator action, not a capability's Bridge call. Probes the real commit (sha/message/best-effort branch/author/files_changed) via `git`, never guessed. The concrete form of "DEP references git commits/branches/tags where relevant, but never depends on git as its storage model" — every other DEP artifact works with zero git involvement; this one piece's whole job is recording that a git action happened, so unlike **resulting state ref**'s `vcs` (optional, silently omitted on failure), a git-commit event fails loudly when there's nothing real to record. |
...
**Verbs.** *resolve* — obtain a canonical implementation from THIS
application's OWN ecosystem root via its own authoritative manifests,
never invent or copy a local equivalent WITHIN an ecosystem that already
has one, and never reach into a DIFFERENT application's ecosystem root
to resolve from there instead of establishing this one's own. This is
distinct from establishing a brand-new application's first canonical
implementation by copying or porting a reference architecture into its
own tree (0-WALKTHROUGH.md section 0, 1-CYCLE.md Phase 0 Gate 35) — that
is how a new ecosystem root comes to have something to resolve in the
first place, not a violation of this verb.
