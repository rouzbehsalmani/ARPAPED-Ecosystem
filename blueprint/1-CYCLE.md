...
- 33 (resume from a checkpoint's own recorded state, never a project-wide
  re-scan — always-applicable, checkpoint.py itself has no Bridge
...
5. Write a resolvable **ecosystem-resolution record** — machine-readable,
   ...
   `ecosystem_resolution_ref`) and, if a checkpoint is now being started
   for this cycle attempt, as its `ecosystem_resolution_ref` too — this is
   the single most valuable thing a checkpoint can save a resuming agent
   from redoing. Written ONCE per ecosystem root, at the moment it is
