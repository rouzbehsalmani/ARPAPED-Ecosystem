...
| **capability** | A named, versioned responsibility that the ecosystem can execute (`capability_id`). Identity is the generic responsibility (`grid.store`; never `<product>.board`) per **R1**. Consumers speak only capability IDs + contract operations (per **R7**). One contract artifact defines it; it may have one or more implementations. |
