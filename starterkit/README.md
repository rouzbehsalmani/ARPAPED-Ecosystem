# Starter kit: logging web app

Copy this. It's the simplest possible complete, runnable starting point
...
capability across both, around a real, keepable domain: logging.

Copy the shape (the folder layout, the file order, the fields, the
request-construction pattern) — the domain itself (`log.write`) is real,
not a placeholder, but its implementation bodies are intentionally
minimal (a line to stdout); expect to swap in real logging behavior
(file output, rotation, structured JSON) behind the same contract.
...
`backend/` and `frontend/` each own their OWN contracts — `client.log`
(frontend) is NOT a second implementation of `log.write` (backend); they
share an operation shape (`message`/`level`) because this starter kit
keeps its domain small, but they're genuinely different capabilities,
with different owners and different responsibilities (`client.log`
reports a client-side event; `log.write` actually writes the line).
What crosses the runtime boundary is a plain reference by capability ID,
never a shared contract file: `client.log` runs **Local** in the
frontend's own Bridge Core and declares `log.write` as a dependency by
ITS OWN contract — a capability it doesn't own and never copies the
contract of. The frontend's own registry has no local implementation of
`log.write` at all, only a manifest entry with `executor_kind: remote`,
naming `/bridge` — and that entry doesn't even reference a contract file
(nothing to read: the owning runtime, backend, is the one authority for
its own shape). Resolving it makes a real HTTP POST to the backend's
`web.serve` capability, shaped by
...
triple). `web.serve` dispatches to `log.write` through the REAL
...
See [`backend/README.md`](backend/README.md) for `log.write`'s own
layout and its direct/out-of-process implementations, and
[`frontend/README.md`](frontend/README.md) for `client.log`'s own layout
and why its code is genuine browser-compatible JavaScript, not Node
pseudocode.
...
the harness itself is scaffolding for the starter kit), then runs
