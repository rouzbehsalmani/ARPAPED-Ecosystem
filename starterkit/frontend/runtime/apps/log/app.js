"use strict";
/**
 * Single request-construction point (blueprint/2-RULES.md R6) for this
 * `log` app -- one of potentially several under runtime/apps/, all
 * sharing the SAME frontend Bridge Core and capability catalog at
 * runtime/ (never duplicated per app) -- the Bridge Adapter (Bridge
 * glossary): builds this runtime's Bridge Core (../../bridge/, this
 * starter kit's own frontend-runtime Bridge -- lives inside
 * frontend/runtime/bridge/, the same way the backend's own Bridge lives
 * inside backend/runtime/bridge/, so this starter kit stays a
 * self-contained, copy-from-here whole: everything a reader needs to
 * copy the shape of one runtime lives under that runtime's own folder,
 * nothing to go hunting for elsewhere in the repo), assembles every
 * implementation from the generated capability catalog (a build
 ...
 * mirror of starterkit/backend/runtime/apps/requests.py.
 *
...
const DECLARED = {
  // This app's own declared-dependencies list (mirrors
  // starterkit/backend/runtime/apps/log/dependencies.yaml) -- keyed by name, not capability_id,
