 ...
 * glossary) -- a faithful port of starterkit/backend/runtime/bridge/bridge.py's own pipeline:
 ...
 *
 * Lives here, inside frontend/ -- this starter kit's own frontend-runtime
 * Bridge, the same way starterkit/backend/runtime/bridge/ lives inside
 * backend/: each runtime's Bridge stays with that runtime, so this
 * starter kit stays one self-contained, copy-from-here whole
 * (README.md), never split across a shared location plus a per-application
...
class BoundCapability {
   ...
   * in starterkit/backend/runtime/bridge/bridge.py -- policy/selection/execution still run
...
/** Mirrors starterkit/backend/runtime/bridge/bridge.py's Dependencies: given once, at assembly
...
/** Minimal constraint matcher for what this starter kit's contracts actually
...
/** Allow-all, mirrors starterkit/backend/runtime/bridge/policy.py's StaticPolicyEngine -- a
 * placeholder decision stage, not a security boundary this starter kit needs
 * to demonstrate. */
function evaluatePolicy(_entry, _policyContext) {
  return { allowed: true };
}

/** Highest-priority-wins, mirrors starterkit/backend/runtime/bridge/selector.py's
...
class BridgeCore {
   ...
   * handles, success or failure -- the JS mirror of starterkit/backend/runtime/bridge/bridge.py's
   ...
   * -> selected -> executed, the same five stages starterkit/backend/runtime/bridge/bridge.py's
