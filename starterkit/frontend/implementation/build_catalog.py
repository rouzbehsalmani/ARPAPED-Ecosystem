...
toward starterkit/backend/runtime/bridge/assembler.py's discipline, just reading THIS runtime's
own JSON manifests (frontend/capabilities/**/manifest.json) instead of
YAML ones, since Node has no built-in YAML parser and this starter kit
adds no npm dependency for it. The frontend Bridge Core (bridge/core.js) never
...
client.log.contract.yaml) against component-contract.schema.json --
...
log.write) has no contract to read here either -- its authoritative
...
_STARTERKIT_ROOT = _FRONTEND_ROOT.parent
_REPO_ROOT = _STARTERKIT_ROOT.parent
_CATALOG_PATH = _FRONTEND_ROOT / "runtime" / "capability-catalog.jsonl"
_MANIFEST_SCHEMA = json.loads((_STARTERKIT_ROOT / "schemas" / "capability-manifest.schema.json").read_text(encoding="utf-8"))
_CONTRACT_SCHEMA = json.loads((_STARTERKIT_ROOT / "schemas" / "component-contract.schema.json").read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    for manifest_path in sorted(_IMPLEMENTATION_ROOT.glob("capabilities/**/manifest.json")):
        ...
        if "contract" in manifest:
            ...
            # (e.g. log.write) -- nothing here to read; the owning
