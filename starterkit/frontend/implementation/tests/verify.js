...
async function main() {
  ...
  const input = { message: "Reported from the frontend's own Bridge", level: "info" };
  try {
    ...
    const check = {
      ...
      input,
      description: `client_log.report reaches every stage through ${response.implementationId}, composing a Remote log.write call`,
      status,
