...
Bridge. A Python capability running as its own separate process would use
...
that currently is; see ../../capabilities/log/write_process/manifest.yaml,
never named here so this comment doesn't go stale when it changes).

Not exercised by a live capability in this starter kit today -- its one
process-kind implementation (log.write.process) is written in C#, so it
uses the OTHER language's reference client, not this one (same posture
as direct_adapter.py, below: prepared ahead of the need, not proven by a
running example yet).
...
(blueprint/2-RULES.md R4) -- and serve it over the wire. A capability's own file
...
Lives inside this starter kit, not under starterkit/backend/runtime/bridge/ (not part of the Bridge
...
starter kit). Uses the standard library's real JSON parser -- the other
...
class Connection:
    ...
    `ProcessExecutorPool` (starterkit/backend/runtime/bridge/process_executor.py). Connects
...
class _Response:
    """The real BoundCapability.call (starterkit/backend/runtime/bridge/bridge.py) returns a
    BridgeResponse with an `.output` attribute -- matched here in case a
    factory closure ever inspects it (none of this starter kit's do)."""
...
class BoundCapability:
    ...
    starterkit/backend/runtime/bridge/bridge.py's real `BoundCapability.call(...)` shape closely
    ...
    def call(self, input: dict[str, Any], policy_context: Any = None) -> _Response:
        ...
        # never carried it explicitly either (starterkit/schemas/process-executor-
...
class Dependencies:
    """Mirrors starterkit/backend/runtime/bridge/bridge.py's real `Dependencies` closely enough
...
def serve_direct(execute: Callable[[str, dict[str, Any], Any], dict[str, Any]]) -> None:
    ...
    (blueprint/2-RULES.md R4), so a leaf capability's code is identical either
...
def serve_factory(make_executor: Callable[[Dependencies], Callable[[str, dict[str, Any], Any], dict[str, Any]]]) -> None:
    ...
    (blueprint/2-RULES.md R4), so a capability that needs to call another
