...
def _apply_input_schema(
    ...
    its declared type (`INPUT_TYPE_CHECKS`, starterkit/backend/runtime/bridge/registry.py) -- and
    ...
    starterkit/backend/runtime/bridge/registry.py) — every one of these is generic and structural,
    ...
    `starterkit/backend/runtime/bridge/assembler.py` already guarantees a declared default satisfies
...
class Dependencies:
    ...
    starterkit/backend/runtime/bridge/assembler.py), scoped to what its own contract declared under
    ...
    version — see `starterkit/backend/runtime/bridge/assembler.py`'s `parse_dependencies`). `resolve`
