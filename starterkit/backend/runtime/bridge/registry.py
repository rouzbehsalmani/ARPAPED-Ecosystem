...
class InputField:
    ...
    read by `starterkit/backend/runtime/bridge/assembler.py`, enforced by `starterkit/backend/runtime/bridge/bridge.py` before
    any executor runs.
...
#: (starterkit/schemas/component-contract.schema.json's input[].type enum). `bool` is
...
#: as if it were `1`. Shared by `starterkit/backend/runtime/bridge/assembler.py` (checking a declared
#: `default`'s own type at assembly time) and `starterkit/backend/runtime/bridge/bridge.py` (checking
