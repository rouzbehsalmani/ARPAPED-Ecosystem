...
Bridge side of `starterkit/schemas/process-executor-protocol.schema.json`; a
...
class ProcessExecutorPool:
    ...
    language's case) -- `starterkit/schemas/capability-manifest.schema.json` allows
    ...
    def _spawn_worker(self, startup_timeout: float) -> _Worker:
        ...
        try:
            ...
            # never prints (one that only makes a nested Bridge call, say)
            # and genuinely non-empty for one whose responsibility IS to
            # print (e.g. log.write.process) -- this pool never assumes
            # either.
