...
with a Dependencies scoped to dependencies.capabilities (log.write,
...
starterkit/schemas/bridge-protocol.schema.json's request/response/error shape
-- the executor_kind: remote wire format (blueprint/2-RULES.md R4) --
dispatching ONLY to log.write (its one declared dependency) via the
Dependencies/nested-call mechanism (Dependencies never imports the
Bridge directly). An API endpoint only -- no static file serving; the
frontend is hosted separately (see
