# Sample: hello world web app — frontend

The JavaScript half of `sample/hello_world` (see `../README.md` for the
...
implementation/                   never read by anything under runtime/ below
  contracts/
    greeting.render.contract.yaml  this runtime's OWN capability, NOT a second implementation of the backend's
                                    greeting.compose (see the contract's own header) — returns the composed
                                    text for display, which greeting.compose never needed to
  capabilities/
    greeting/render/manifest.json  depends on console.write BY ID (R4), never on the backend's contract file
    console/write/manifest.json    no `contract` field at all — a Remote-kind pointer to a capability this
...
runtime/                          everything a real deployment of this half needs — both what a client
  ...
  capabilities/greeting/render/executor.js   the ONLY file this capability ships — no manifest.json here
...
`greeting.render` runs **Local** in this runtime's own Bridge Core and
declares `console.write` as a dependency, pinned `<2.0.0` by ITS OWN
...
`console.write` at all, only a manifest entry with `executor_kind:
...
policy)` triple). `web.serve` dispatches to `console.write` through the
...
catalog is a generated build artifact, committed so the sample runs out
of the box.
...
runtime's own `console.write` dependency is Remote, reached only by
calling that API.
...
exists for a real browser loading real HTTP responses. 16/16 passing
...
adjust it if you started the API on a different port. Type a name, click
**Greet**: `greeting.render` executes **Local**, right there in the
page, and its `console.write` dependency makes a real cross-origin HTTP
POST to the backend's `/bridge` endpoint — watch the terminal running
the backend's `main.py`; that's where the text actually gets printed, by
the backend's own `console.write`, proving the round trip is real and
not two independent halves that happen to sit next to each other.

Everything the static server above serves to a client (`index.html`,
`app.js`, `bridge/*.js`, `capabilities/greeting/render/executor.js`,
