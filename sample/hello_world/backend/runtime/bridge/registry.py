...
blueprint/2-RULES.md "Registry contract"), stopping at the first level that returns a
...
class InputField:
    ...
    read by `sample/hello_world/backend/runtime/bridge/assembler.py`, enforced by `sample/hello_world/backend/runtime/bridge/bridge.py` before
    any executor runs.
...
#: (sample/schemas/component-contract.schema.json's input[].type enum). `bool` is
...
#: as if it were `1`. Shared by `sample/hello_world/backend/runtime/bridge/assembler.py` (checking a declared
#: `default`'s own type at assembly time) and `sample/hello_world/backend/runtime/bridge/bridge.py` (checking
...
class CapabilityRegistry:
    ...
    def discover_cascade(
        ...
        *,
        ...
        Levels, in order (blueprint/2-RULES.md "Registry contract"):
