...
def _apply_input_schema(
    ...
    (blueprint/2-RULES.md "No silent defaults on what resolution depends on") --
    required fields present, and a present field's runtime value matching
    its declared type (`INPUT_TYPE_CHECKS`, sample/hello_world/backend/runtime/bridge/registry.py) -- and
    ...
    sample/hello_world/backend/runtime/bridge/registry.py) — every one of these is generic and structural,
    ...
    `sample/hello_world/backend/runtime/bridge/assembler.py` already guarantees a declared default satisfies
...
class Dependencies:
    ...
    sample/hello_world/backend/runtime/bridge/assembler.py), scoped to what its own contract declared under
    ...
    version — see `sample/hello_world/backend/runtime/bridge/assembler.py`'s `parse_dependencies`). `resolve`
