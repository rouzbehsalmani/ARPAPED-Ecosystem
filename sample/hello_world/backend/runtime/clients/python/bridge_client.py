...
class Connection:
    ...
    `ProcessExecutorPool` (sample/hello_world/backend/runtime/bridge/process_executor.py). Connects
...
class _Response:
    """The real BoundCapability.call (sample/hello_world/backend/runtime/bridge/bridge.py) returns a
...
class BoundCapability:
    ...
    sample/hello_world/backend/runtime/bridge/bridge.py's real `BoundCapability.call(...)` shape closely
    ...
    def call(self, input: dict[str, Any], policy_context: Any = None) -> _Response:
        ...
        # never carried it explicitly either (sample/schemas/process-executor-
...
class Dependencies:
    """Mirrors sample/hello_world/backend/runtime/bridge/bridge.py's real `Dependencies` closely enough
