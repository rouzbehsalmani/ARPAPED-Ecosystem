...
independently adoptable pillars"): [`runtime.yaml`](runtime.yaml),
[`evolution.yaml`](evolution.yaml), [`process.yaml`](process.yaml).
...
proven by this repo's own test suite (`packs/evolution.yaml`'s
...
(e.g. Runtime + Evolution's wiring is documented once, in
`runtime.yaml`'s `combine_with.evolution`, not repeated in
`evolution.yaml`'s `combine_with.runtime` — read both, they're short).

For the full picture of Process + Runtime combined specifically — which
...
This works even without the Process pillar adopted for the actual
application: a first agent can still prepare a Runtime+Evolution
